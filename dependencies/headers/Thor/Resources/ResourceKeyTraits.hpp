...
namespace detail
{
	...
	struct KeyInfoAccessor
	{
		...
		template <class Key>
		static const std::string& Get(const Key& key, Int2Type<true>)
		{
			return key.mKey;
		}
		...
		template <class Key>
		static const std::string& Get(const Key&, Int2Type<false>)
		{
	...
	const std::string& GetKeyInfo(const Key& key)
	{
		...
		return KeyInfoAccessor::Get(key, Int2Type<builtin>());
	}
