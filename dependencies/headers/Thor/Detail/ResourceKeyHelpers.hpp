...
namespace detail
{
	...
	MovedPtr<Resource, NoCopy> LoadResource(const tr1::function<bool(Resource&)>& loader)
	{
		MovedPtr<Resource, NoCopy> resource(new Resource);

		if (loader(*resource))
			return resource;
		else
			return MovedPtr<Resource, NoCopy>();
	}
